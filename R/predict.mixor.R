predict.mixor<-function (object, newdata = NULL, na.action = na.fail, ...) 
{
    y <- object$Y
    link <- object$link
    IADD <- object$IADD
    ...
    if (IADD == 1) {
        if (object$random.effect.mean == TRUE & ICEN == 0) {
            alpha <- c(as.numeric(object$MU[1]), as.numeric(object$MU[1]) + 
                as.numeric(object$GAM)[1:(k - 2)])
        }
        else if (object$random.effect.mean == TRUE & ICEN == 
            1) {
            alpha <- c(as.numeric(object$MU[1]), as.numeric(object$MU[1]) + 
                as.numeric(object$GAM)[1:(k - 1)])
        }
        else {
    ...
    tt <- terms(object)
    if (!inherits(object, "mixor")) 
        warning("calling predict.mixor(<fake-lm-object>) ...")
    if (missing(newdata) || is.null(newdata)) {
        ...
        if (!is.null(cl <- attr(Terms, "dataClasses"))) 
            .checkMFClasses(cl, m)
        mf <- object$call
        mfmatch <- match(c("formula", "data", "id", "subset", 
            "weights"), names(mf), 0L)
        IDS <- newdata[, as.character(mf[["id"]])]
        X <- model.matrix(Terms, m, contrasts.arg = object$contrasts)
        random<-match(colnames(object$X),colnames(X))
        newx<-X[,random, drop=FALSE]
        neww<-X[,-random, drop=FALSE]
	}
    n <- dim(neww)[1]
    ...
    if (missing(newdata) || is.null(newdata)) {
        if (dim(object$EBmean)[2] == 1) {
            if (dim(newx)[2] == 1) {
                theta <- as.matrix(rep(object$EBmean, reps), 
                  ncol = 1)
                ...
                for (i in 1:length(unique.id)) {
                  ...
                  if (!is.null(which.random.slope)) {
                    ...
                    theta <- rep(object$EBmean[i, ], sum(dimnames(newx)[[1]] == 
                      unique.id[i]))
                  }
                  Xb[dimnames(neww)[[1]] == unique.id[i], ] <- w %*% 
                    zeta + x %*% theta
                }
            }
        }
    ...
    for (i in 1:(k - 1)) {
        if (IADD == 1) {
            if (length(alpha) == 0L & KG == 0 & KS == 0) {
                ...
                new.thresh <- c(0, as.numeric(object$GAM)[-(1:(k - 
                  2))])
                if (KG >= 2) {
                  interact <- apply(new.thresh[seq(i, length(new.thresh), 
                    by = 2)] * neww[, 1:KG], 1, sum)
                }
                else {
    ...
    if (ICEN == 0) {
        ...
        dimnames(pi)[[2]] <- c(sort(unique(y)), paste(sort(unique(y))[k - 
            1], "+", sep = ""))
    }
    if (link == "logit") {
        for (i in 1:k) {
            if (i == 1) {
                ...
                pi[, i] <- exp(z[, i])/(1 + exp(z[, i])) - exp(z[, 
                  i - 1])/(1 + exp(z[, i - 1]))
            }
            else if (i == k) {
                pi[, i] <- 1 - exp(z[, i - 1])/(1 + exp(z[, i - 
                  1]))
            }
        }
    }
